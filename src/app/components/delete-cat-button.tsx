...
export default function DeleteCatButton({ catId }: { catId: number }) {
  ...
  return (
    <>
      ...
      <Dialog fullScreen open={isModalOpen} onClose={() => setIsModalOpen(!isModalOpen)}>
        <DialogTitle>Delete Cat?</DialogTitle>
        <DialogContent>
          <DialogContentText>
            This action is probably <span className="text-white">irreversible </span>but your cat will be unharmed.</DialogContentText>
        </DialogContent>
        <DialogActions sx={{ justifyContent: "space-between" }}>
          <Button onClick={() => setIsModalOpen(!isModalOpen)} variant="contained">
            Cancel
          </Button>
          <LoadingButton loading={deleteCatMutation.isPending} variant="outlined" onClick={() => deleteCatMutation.mutate()} color="error" size="small">
            Delete Cat
          </LoadingButton>
        </DialogActions>
      </Dialog>
